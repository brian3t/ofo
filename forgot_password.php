...
*/
	...
	$current_page = "forgot_password.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("forgot_password", $layout_id);
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "forgot_password") {
				include_once("./blocks/block_password_forgot.php");
				password_forgot_form($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", FORGOT_PASSWORD_MSG));
