...
*/
	...
	$current_page = "support_messages.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("support_reply", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "support_messages.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "support_reply") {
				include_once("./blocks/block_support_reply.php");
				support_reply($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", SUPPORT_TITLE));
