...
/*
  ...
  ***                                                                      ***
  ***      ViArt Shop 3.6                                                  ***
  ***      File:  ideal_confirm_cronjob.php                                ***
...
/*
 * iDEAL (www.ing-ideal.nl) Cronjob transaction handler by www.viart.com
 */
	...
	$is_ssl = (strtoupper(get_var("HTTPS")) == "ON");
	...
	function set_status()
	{
		...
		if ("SHA1_RSA" == $payment_params["AuthenticationType"]) {
			...
			$message = stripsimbls( $message );

			$token = createCertFingerprint($payment_params["Privatecert"]);
			$tokenCode = signMessage( $payment_params["Privatekey"], $payment_params["PrivatekeyPass"], $message );
		...
		$answer = PostToHost($payment_params["AcquirerURL"], $payment_params["AcquirerTimeout"], $reqMsg);
		...
		$valid = verifyMessage($payment_params["Certificate0"], $message, $sig );
