...
*/
	...
	$current_page = "polls.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("polls_previous", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "polls.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "polls_previous_list") {
				include_once("./blocks/block_polls_previous_list.php");
				polls_previous_list($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", PREVIOUS_POLLS_MSG));
