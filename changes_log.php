...
*/
	...
	$current_page = "changes_log.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("products_changes_log", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "changes_log.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "products_changes_log") {
				include_once("./blocks/block_products_changes_log.php");
				products_changes_log($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", CHANGES_LOG_TITLE));
