...
*/
	...
	$current_page = "cart_retrieve.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("cart_retrieve", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "cart_retrieve.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "cart_retrieve") {
				include_once("./blocks/block_cart_retrieve.php");
				cart_retrieve($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", RETRIEVE_CART_TITLE));
