...
function categories($block_name, $block_prefix="categories", $block_body="block_categories_list.html")
{
	...
	if (($categories_type == 2)||($categories_type == 1))
	{
		...
		if ($categories_type == 1) {
			$sql  = " SELECT category_id AS top_category_id, category_name AS top_category_name, ";
		...
		$t->set_file("block_body", $block_body);
		...
		if ($categories_type == 4) { // Tree-type structure
			...
			$categories_ids = VA_Categories::find_all_ids("c.parent_category_id IN (" . $db->tosql($category_path, INTEGERS_LIST) . ")", VIEW_CATEGORIES_PERM);
			$allowed_categories_ids = VA_Categories::find_all_ids("c.parent_category_id IN (" . $db->tosql($category_path, INTEGERS_LIST) . ")", VIEW_CATEGORIES_ITEMS_PERM);
		} else {
		...
		$categories = array();
		$sql  = " SELECT category_id, category_name, friendly_url, ";
		...
		while ($db->next_record()) {
			...
			$short_description = get_translation($db->f("short_description"), $language_code);
			$image = $db->f("image");
			$image_alt = get_translation($db->f("image_alt"));
		...
		if (sizeof($categories) > 0 && isset($categories[0]))
		{
			...
			if($block_body == "block_categories_footer.html") {
				$category_style = "list";
			} else {
				$category_style = "nested";
			}

			set_categories(0, 0, $columns, 0, $categories_image, "", $category_style);
