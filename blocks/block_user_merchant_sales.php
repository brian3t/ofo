<?php
	...
	$t->set_file("block_body","block_user_merchant_sales.html");
	...
	$t->set_var("user_merchant_items_href",get_custom_friendly_url("user_merchant_items.php"));
	...
	$s = new VA_Sorter($settings["templates_dir"], "sorter_img.html", get_custom_friendly_url("user_merchant_sales.php"));
	$s->set_parameters(false, true, true, false);
	if ($db_type == "access" || $db_type == "postgre") {
		$t->set_var("sorter_time", TIME_PERIOD_MSG);
		$s->set_sorter(ITEMS_NUMBER_COLUMN, "sorter_products_qty", "3", "SUM(oi.quantity)");
		$s->set_sorter(GROSS_SALES_MSG, "sorter_sales", "4", "SUM(oi.price * oi.quantity)");
		$s->set_sorter(FEES_MSG, "sorter_fees", "5", "(SUM(oi.price * oi.quantity) - SUM(oi.merchant_commission * oi.quantity))");
		$s->set_sorter(NET_SALES_MSG, "sorter_net_sales", "6", "SUM(oi.merchant_commission * oi.quantity)");
	} elseif ($db_type == "mysql") {
		$s->set_default_sorting(1, "asc");
		$s->set_sorter(TIME_PERIOD_MSG, "sorter_time", "1", "order_placed_date");
		$s->set_sorter(ITEMS_NUMBER_COLUMN, "sorter_products_qty", "3", "products_qty");
		$s->set_sorter(GROSS_SALES_MSG, "sorter_sales", "4", "sales");
		$s->set_sorter(FEES_MSG, "sorter_fees", "5", "fees");
		$s->set_sorter(NET_SALES_MSG, "sorter_net_sales", "6", "net_sales");
	}
	...
	$sum_products_qty = 0;
	$sum_sales = 0; $sum_fees = 0; $sum_net_sales = 0;
	...
	$sql .= " (SUM(oi.price * oi.quantity) - SUM(oi.merchant_commission * oi.quantity)) AS fees, ";
	$sql .= " SUM(oi.merchant_commission * oi.quantity) AS net_sales  ";
	...
	if ($db->next_record()) {
		...
		do {
			...
			$fees = doubleval($db->f("fees"));
			$net_sales = doubleval($db->f("net_sales"));
			...
			$t->set_var("fees", currency_format($fees));
			$t->set_var("net_sales", currency_format($net_sales));
		...
		$t->set_var("sum_fees", currency_format($sum_fees));
		$t->set_var("sum_net_sales", currency_format($sum_net_sales));
