<?php
	...
	$t->set_file("block_body","block_user_merchant_items.html");
	...
	$s = new VA_Sorter($settings["templates_dir"], "sorter_img.html", get_custom_friendly_url("user_merchant_items.php"));
	...
	if ($db_type == "access") {
		...
		$s->set_sorter(FEES_MSG, "sorter_fees", "6", "(SUM(oi.price * oi.quantity) - SUM(oi.merchant_commission * oi.quantity))");
		$s->set_sorter(NET_SALES_MSG, "sorter_net_sales", "7", "SUM(oi.merchant_commission * oi.quantity)");
	} else {
		$s->set_default_sorting(4, "desc");
		$s->set_sorter(ID_MSG, "sorter_id", "1", "item_id");
		$s->set_sorter(PROD_NAME_MSG, "sorter_name", "2", "item_name");
		...
		$s->set_sorter(FEES_MSG, "sorter_fees", "6", "fees");
		$s->set_sorter(NET_SALES_MSG, "sorter_net_sales", "7", "net_sales");
	}
	...
	$sum_qty = 0; $sum_sales = 0; $sum_avg_price = 0; $sum_fees = 0; $sum_net_sales = 0;
