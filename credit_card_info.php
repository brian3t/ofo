...
*/
	...
	$current_page = "credit_card_info.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("order_payment_details", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main","credit_card_info.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "checkout_breadcrumb") {
				include_once("./blocks/block_checkout_breadcrumb.php");
				checkout_breadcrumb($setting_value);
			} elseif ($setting_name == "order_cart") {
				include_once("./blocks/block_order_cart.php");
				order_cart($setting_value);
			} elseif ($setting_name == "order_payment_details_form") {
				$block_name = $setting_value;
				include_once("./blocks/block_order_payment_form.php");
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", CHECKOUT_PAYMENT_TITLE));
