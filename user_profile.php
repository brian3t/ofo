...
*/
	...
	$layout_id = 0;
	$page_settings = va_page_settings("user_profile", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "user_profile.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "user_profile_form") {
				include_once("./blocks/block_user_profile.php");
				user_profile_form($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", PROFILE_TITLE));
