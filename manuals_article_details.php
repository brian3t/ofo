...
*/
	...
	$page_name = "manuals_article_details";
	$page_settings = va_page_settings($page_name, 0);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main","manuals_article_details.html");
	$t->set_var("current_href", "manuals_article_details.php");
	...
	if (is_array($page_settings)) {
		foreach($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "manuals_articles") {
				include_once("./blocks/block_manuals_articles.php");
				manuals_articles($setting_value);
			} elseif ($setting_name == "manuals_article_details") {
				include_once("./blocks/block_manuals_article_details.php");
				manuals_article_details($setting_value);
