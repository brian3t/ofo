...
*/
	...
	$current_page = "order_confirmation.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("order_confirmation", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main","order_confirmation.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "order_data_preview") {
				$block_name = $setting_value;
				include_once("./blocks/block_order_data_preview.php");
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", CHECKOUT_CONFIRM_TITLE));
