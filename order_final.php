...
*/
	...
	$current_page = "order_final.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("order_final", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main","order_final.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "checkout_final") {
				$block_name = $setting_value;
				include_once("./blocks/block_checkout_final.php");
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", CHECKOUT_FINAL_TITLE));
