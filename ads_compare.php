...
*/
	...
	$current_page = "ads_compare.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("ads_compare", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "ads_compare.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "ads_compare") {
				include_once("./blocks/block_ads_compare.php");
				ads_compare($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", ADS_COMPARE_TITLE));
