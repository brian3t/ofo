...
<div>
<?php
	if(!empty( $_POST ) && $_POST['action'] == 'import_thesis' && $nonce = $_POST['nonce-aioseopi-migrate']){
...
<div style="float:left;background-color:white;padding: 10px 10px 10px 10px;margin-right:15px;border: 1px solid #ddd;">
	...
	$thesistitlecount = $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='thesis_title';");
	$thesisdesccount = $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='thesis_description';");
	$thesiskeywordscount = $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key='thesis_keywords';");
