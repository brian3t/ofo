...
*/
	...
	$current_page = "user_affiliate_sales.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("userhome_pages", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "user_home_page.html");
	...
	if (is_array($page_settings)) {
		...
		{
			if ($setting_name == "userhome_breadcrumb") {
				include_once("./blocks/block_userhome_breadcrumb.php");
				userhome_breadcrumb($setting_value);
			} elseif ($setting_name == "userhome_main_block") {
				$block_name = $setting_value;
				include_once("./blocks/block_user_affiliate_sales.php");
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", AFFILIATE_SALES_MSG));
