...
*/
	...
	$current_page = "user_login.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("user_login", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "user_login.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "advanced_login") {
				include_once("./blocks/block_login_advanced.php");
				login_advanced_form($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", LOGIN_TITLE));
