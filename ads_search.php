...
*/
	...
	$layout_id = 0;
	$page_settings = va_page_settings("ads_search", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "ads_search.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "ads_search_advanced") {
				include_once("./blocks/block_ads_search_advanced.php");
				ads_search_advanced($setting_value);
