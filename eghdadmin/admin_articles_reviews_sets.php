...
/*
  ...
  ***                                                                      ***
  ***      ViArt Shop 3.6                                                  ***
  ***      File:  admin_articles_reviews_sets.php                          ***
...
*/
	...
	check_admin_security("articles_reviews_settings");
	...
	$t->set_file("main", "admin_articles_reviews_sets.html");

	include_once("./admin_header.php");

	$t->set_var("admin_href", "admin.php");
	$t->set_var("admin_articles_top_href", "admin_articles_top.php");
	$t->set_var("admin_articles_reviews_sets_href", "admin_articles_reviews_sets.php");
	...
	if (strlen($operation))
	{
		...
		if (!strlen($r->errors))
		{
			$sql  = " DELETE FROM " . $table_prefix . "global_settings WHERE setting_type='articles_reviews'";
			...
			foreach ($r->parameters as $key => $value)
			{
				...
				$sql .= "'articles_reviews', '" . $key . "'," . $db->tosql($value[CONTROL_VALUE], TEXT) . ",";
	...
	else // get articles_reviews settings
	{
		foreach ($r->parameters as $key => $value)
		{
			$sql  = " SELECT setting_value FROM " . $table_prefix . "global_settings ";
			$sql .= " WHERE setting_type='articles_reviews' AND setting_name='" . $key . "'";
