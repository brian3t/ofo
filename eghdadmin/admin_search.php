...
*/
	...
	check_admin_security("advanced_search");

  $t = new VA_Template($settings["admin_templates_dir"]);
  $t->set_file("main","admin_search.html");
	...
	$r->add_checkbox("manufacturer", INTEGER);
	$r->add_checkbox("price", INTEGER);
	$r->add_checkbox("weight", INTEGER);
	...
	if(strlen($operation))
	{
		...
		if(!strlen($r->errors))
		{
			$sql  = " DELETE FROM " . $table_prefix . "global_settings WHERE setting_type LIKE 'search%'";
			...
			foreach($r->parameters as $key => $value)
			{
				...
				$sql .= "'search', '" . $key . "'," . $db->tosql($value[CONTROL_VALUE], TEXT) . ",";
			...
			for ($i = 1; $i <= $properties_number; $i++) {
				...
				if ($property_show == 1) {
					$sql  = "INSERT INTO " . $table_prefix . "global_settings (setting_type, setting_name, setting_value, site_id) VALUES (";
					$sql .= "'search_properties', " . $db->tosql($property_name, TEXT) . "," . $db->tosql(1, TEXT) . ",";
					$sql .= $db->tosql($param_site_id,INTEGER) . ") ";
					$db->query($sql);
				}
				$sql  = "INSERT INTO " . $table_prefix . "global_settings (setting_type, setting_name, setting_value, site_id) VALUES (";
				$sql .= "'search_properties_values', " . $db->tosql($property_name, TEXT) . "," . $db->tosql($property_value, TEXT) . ",";
				$sql .= $db->tosql($param_site_id,INTEGER) . ") ";
				$db->query($sql);
			}
			...
			for ($i = 1; $i <= $features_number; $i++) {
				...
				if ($feature_show == 1) {
					$sql  = "INSERT INTO " . $table_prefix . "global_settings (setting_type, setting_name, setting_value, site_id) VALUES (";
					$sql .= "'search_features', " . $db->tosql($feature_name, TEXT) . "," . $db->tosql(1, TEXT) . ",";
					$sql .= $db->tosql($param_site_id,INTEGER) . ") ";
					$db->query($sql);					
				}
				$sql  = "INSERT INTO " . $table_prefix . "global_settings (setting_type, setting_name, setting_value, site_id) VALUES (";
				$sql .= "'search_features_values', " . $db->tosql($feature_name, TEXT) . "," . $db->tosql($feature_value, TEXT) . ",";
				$sql .= $db->tosql($param_site_id,INTEGER) . ") ";
				$db->query($sql);
			}
  ...
  $dbs->DBPersistent = $db->DBPersistent;
	...
	if ($db->next_record()) {
		...
		do {
			...
			$t->set_var("property_name_translation", htmlspecialchars(get_translation($property_name)));
	...
	$features_number = 0;
	$sql  = " SELECT feature_name FROM " . $table_prefix . "features f ";
	...
	if ($db->next_record()) {
		...
		do {
			...
			if (isset($features_values[$feature_name])) {
				$feature_value = $features_values[$feature_name];
			} else {
				$sql  = " SELECT feature_value FROM " . $table_prefix . "features_default ";
				$sql .= " WHERE feature_name=" . $db->tosql($feature_name, TEXT);
				$dbs->query($sql);
				if ($dbs->next_record()) {
					$feature_value = $dbs->f("feature_value");
				} else {
					$feature_value = "";
				}
			}
			$t->set_var("feature_name", htmlspecialchars($feature_name));
			$t->set_var("feature_name_translation", htmlspecialchars(get_translation($feature_name)));
