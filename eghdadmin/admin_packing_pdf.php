...
*/
	...
	function set_user_info()
	{
		...
		if ($delivery_number > 0) {
			$pdf->show_xy(DELIVERY_TO_MSG.":", 350, $height_position, 250, 0, "left");
		}
		...
		if (strlen($delivery_name)) {
			$delivery_height -= 12;
			$pdf->show_xy($delivery_name, 350, $delivery_height, 250, 0, "left");
		}
	
		if ($r->parameters["delivery_company_id"][SHOW] && $r->get_value("delivery_company_id")) {
			$delivery_height -= 12;
			$pdf->show_xy($r->get_value("delivery_company_id"), 350, $delivery_height, 250, 0, "left");
		}
		if ($r->parameters["delivery_company_name"][SHOW] && $r->get_value("delivery_company_name")) {
			$delivery_height -= 12;
			$pdf->show_xy($r->get_value("delivery_company_name"), 350, $delivery_height, 250, 0, "left");
		}
	
		if ($r->parameters["delivery_address1"][SHOW] && $r->get_value("delivery_address1")) {
			$delivery_height -= 12;
			$pdf->show_xy($r->get_value("delivery_address1"), 350, $delivery_height, 250, 0, "left");
		}
		if ($r->parameters["delivery_address2"][SHOW] && $r->get_value("delivery_address2")) {
			$delivery_height -= 12;
			$pdf->show_xy($r->get_value("delivery_address2"), 350, $delivery_height, 250, 0, "left");
		}
		...
		if (strlen($delivery_address)) {
			$delivery_height -= 12;
			$pdf->show_xy($delivery_address, 350, $delivery_height, 250, 0, "left");
		}
	
		if ($r->parameters["delivery_country_id"][SHOW] && $r->get_value("delivery_country_id")) {
			$delivery_height -= 12;
			$pdf->show_xy($r->get_value("delivery_country_id"), 350, $delivery_height, 250, 0, "left");
		}
	
		if ($r->parameters["delivery_phone"][SHOW] && !$r->is_empty("delivery_phone")) {
			$delivery_height -= 12;
			$pdf->show_xy(PHONE_FIELD.": ".$r->get_value("delivery_phone"), 350, $delivery_height, 250, 0, "left");
		}
		if ($r->parameters["delivery_daytime_phone"][SHOW] && !$r->is_empty("delivery_daytime_phone")) {
			$delivery_height -= 12;
			$pdf->show_xy(DAYTIME_PHONE_FIELD.": ".$r->get_value("delivery_daytime_phone"), 350, $delivery_height, 250, 0, "left");
		}
		if ($r->parameters["delivery_evening_phone"][SHOW] && !$r->is_empty("delivery_evening_phone")) {
			$delivery_height -= 12;
			$pdf->show_xy(EVENING_PHONE_FIELD.": ".$r->get_value("delivery_evening_phone"), 350, $delivery_height, 250, 0, "left");
		}
		if ($r->parameters["delivery_cell_phone"][SHOW] && !$r->is_empty("delivery_cell_phone")) {
			$delivery_height -= 12;
			$pdf->show_xy(CELL_PHONE_FIELD.": ".$r->get_value("delivery_cell_phone"), 350, $delivery_height, 250, 0, "left");
		}
		if ($r->parameters["delivery_fax"][SHOW] && !$r->is_empty("delivery_fax")) {
			$delivery_height -= 12;
			$pdf->show_xy(FAX_FIELD.": ".$r->get_value("delivery_fax"), 350, $delivery_height, 250, 0, "left");
		}
	
		if ($r->parameters["delivery_email"][SHOW] && strlen($r->get_value("delivery_email"))) {
			$delivery_height -= 12;
			$pdf->show_xy(EMAIL_FIELD.": " . $r->get_value("delivery_email"), 350, $delivery_height, 250, 0, "left");
		}
		// Egghead Ventures Add
		$delivery_height -= 12;
		$pdf->show_xy("Shipping Method: " . $r->get_value("shipping_type_desc"), 350, $delivery_height, 250, 0, "left");
		// End
	...
	function set_packing_header()
	{
		...
		$height_position -= 50;
		$pdf->setfont("helvetica", "B", 10);
		$pdf->show_xy(INVOICE_DATE_MSG . ":", 350, $height_position, 80, 0, "left");
		$pdf->setfont("helvetica", "", 10);
		$pdf->show_xy($order_date, 430, $height_position, 200, 0, "left");
	
		$height_position -= 12;
		$pdf->setfont("helvetica", "B", 10);
		$pdf->show_xy(INVOICE_NUMBER_MSG . ":", 350, $height_position, 80, 0, "left");
		$pdf->setfont("helvetica", "", 10);
		$pdf->show_xy($invoice_number, 430, $height_position, 200, 0, "left");
