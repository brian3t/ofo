...
*/
	...
	$t->set_file("main","admin_order_links.html");
	...
	$send_links = get_param("send_links");
	...
	if(strlen($operation))
	{
		...
		if($is_valid) {
			...
			if(!$message_sent) {
				...
				$oe->set_value("event_type", "links_sent");
		...
		$mail_from = get_setting_value($download_info, "links_from", $settings["admin_email"]);
		$mail_cc = get_setting_value($download_info, "links_cc");
		$mail_bcc = get_setting_value($download_info, "links_bcc");
		$mail_reply_to = get_setting_value($download_info, "links_reply_to");
		$mail_return_path = get_setting_value($download_info, "links_return_path");
		$mail_type = get_setting_value($download_info, "links_message_type");
	  ...
	  // get customer email
		...
		if(isset($download_info["links_subject"]) && $download_info["links_subject"]) {
			$links_subject = $download_info["links_subject"];
		} else {
			$links_subject = LINKS_FOR_ORDER_MSG . $order_id;
		}

		if(isset($download_info["links_message"]) && $download_info["links_message"]) {
			$links_message = $download_info["links_message"];
		} else {
			$links_message = $links;
		}

		$t->set_block("message_subject", $links_subject);
		$t->set_block("message_body", $links_message);
		...
		$delivery_country = get_db_value("SELECT country_name FROM " . $table_prefix . "countries WHERE country_id=" . $db->tosql($delivery_country_id, INTEGER, TEXT));
	...
	$s = new VA_Sorter($settings["admin_templates_dir"], "sorter_img.html", "admin_order_links.php");
	$s->set_sorter(ID_MSG, "sorter_download_id", "1", "download_id");
	$s->set_sorter(PRODUCT_MSG, "sorter_item_name", "2", "item_name");
	$s->set_sorter(ADMIN_ACTIVATED_MSG, "sorter_activated", "3", "activated");
	...
	$sql  = " SELECT id.download_id,oi.item_name,id.download_path,id.activated,id.download_added FROM " . $table_prefix . "items_downloads id, " . $table_prefix . "orders_items oi ";
	...
	if($db->next_record())
	{
		...
		do {
			...
			$t->set_var("file_name", basename($download_path));
			$t->set_var("download_id", $download_id);
			$t->set_var("download_url", $item_download_url);
