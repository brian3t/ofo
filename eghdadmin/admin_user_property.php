...
*/
	...
	$sql = " SELECT type_name FROM " . $table_prefix . "user_types WHERE type_id=" . $db->tosql($user_type_id, INTEGER);
	$db->query($sql);
	if ($db->next_record()) {
	...
	$return_page = "admin_user_profile.php?type_id=" . $user_type_id . "&tab=custom_fields";
	...
	if (strlen($operation) && !$more_properties)
	{
		...
		if ($is_valid)
		{
			...
			{
				$db->query("SELECT MAX(property_id) FROM " . $table_prefix . "user_profile_properties");
				$db->next_record();
