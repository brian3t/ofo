...
*/
	...
	$current_user_type_id = ""; $current_subscription_id = ""; $current_is_approved = 0;
	$sql  = " SELECT user_type_id, is_approved, subscription_id, ";
	$sql .= " login, name, first_name, last_name ";
	$sql .= " FROM " . $table_prefix . "users ";
	$sql .= " WHERE user_id=" . $db->tosql($user_id, INTEGER);
	$db->query($sql);
	if ($db->next_record()) {
		$current_user_type_id = $db->f("user_type_id");
		$current_is_approved = $db->f("is_approved");
		$current_subscription_id = $db->f("subscription_id");
		$login = $db->f("login");
		$user_name = $db->f("name");
		if (!$user_name) { $user_name = trim($db->f("first_name") . " " . $db->f("last_name")); }
	...
	$t->set_file("main","admin_user_change_type.html");
	...
	$t->set_var("admin_user_change_type_href", "admin_user_change_type.php");
	...
	if(strlen($operation))
	{
		...
		if (!$update_users) {
			$errors .= UPDATE_ALLOWED_ERROR . "<br>";
		}
		...
		if (!$errors) {
			...
			if ($current_user_type_id != $type_id) {
				// redirect to user edit page if type changed
				header("Location: admin_user.php?user_id=".$user_id);
				exit;
			} else {
				// redirect to user edit page if type changed
				header("Location: admin_users.php");
