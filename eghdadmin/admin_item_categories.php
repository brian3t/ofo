...
/*
  ...
  ***                                                                      ***
  ***      ViArt Shop 3.6                                                  ***
  ***      File:  admin_item_categories.php                                ***
...
*/
	...
	check_admin_security("product_categories");
	...
	$t->set_file("main","admin_item_categories.html");
	...
	elseif ($operation == "save")
	{
		...
		if (!strlen($errors))
		{
			...
			$session_used_categories = get_session("session_recently_used_categories");
			...
			$sql .= " WHERE item_id=" . $db->tosql($item_id, INTEGER);
			if ($categories_ids) {
				$sql .= " AND category_id NOT IN (" . $db->tosql($categories_ids, INTEGERS_LIST) . ")";
			}
			$db->query($sql);
			...
			if ($categories_ids) {
				...
				$sql .= " WHERE item_id=" . $db->tosql($item_id, INTEGER);
				$sql .= " AND category_id IN (" . $db->tosql($categories_ids, INTEGERS_LIST) . ")";
				$db->query($sql);
				...
				$categories_ids= array_values(array_diff($categories_ids, $existing_categories));
				
				if ($categories_ids) {
					for ($i = 0; $i < sizeof($categories_ids); $i++) {
						$sql  = " SELECT MAX(item_order) FROM " . $table_prefix . "items_categories ";
						$sql .= " WHERE category_id=" . $db->tosql($categories_ids[$i], INTEGER);
						...
						$sql .= $db->tosql($categories_ids[$i], INTEGER) . ",";
			...
			set_session("session_recently_used_categories", $recently_used_categories);
	...
	if ($tab == "all") {
				...
				for ($i = 0; $i < sizeof($path_ids); $i++) {
					...
					if ($path_id) {
						if (!isset($categories[$path_id])) {
							$sql  = " SELECT category_name, parent_category_id, category_path FROM " . $table_prefix . "categories ";
		...
		$sql .= " LEFT JOIN " . $table_prefix . "categories sc ON c.category_id=sc.parent_category_id) ";
	...
	// parse tabs
	$tab_url = new VA_URL("admin_item_categories.php", false);
