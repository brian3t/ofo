...
*/
	...
	$layout_id = 0;
	$page_settings = va_page_settings("cart_save", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "cart_save.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "cart_save") {
				include_once("./blocks/block_cart_save.php");
				cart_save($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", SAVE_CART_TITLE));
