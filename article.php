...
*/
	...
	if ($db->next_record()) {
		...
		if ($parent_category_id == 0) {
			...
			if (!@file_exists($details_template)) { $details_template = ""; }
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			if (preg_match("/^a_details_(\d+)$/", $setting_name, $matches)) {
				...
				related_articles($setting_value, "articles_related", $page_friendly_url, $page_friendly_params);
			...
			} elseif (preg_match("/^a_forums_related_(\d+)$/", $setting_name, $matches)) {
				include_once("./blocks/block_forums_related.php");
				related_forums($setting_value, "articles_forum_topics", $page_friendly_url, $page_friendly_params);
