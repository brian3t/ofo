...
*/
	...
	if ($category_path) {
		...
		for ($c = 0; $c < sizeof($ids); $c++) {
			$id = $ids[$c];
			if ($where) { $where .= " OR "; }
			$where .= "page_name=" . $db->tosql("products_list_" . $id, TEXT);		
		}
		$layout_types = array();
		if ($where) {
			$sql  = " SELECT page_name, setting_value FROM " . $table_prefix . "page_settings ";
			if (isset($site_id) && $site_id != 1) {
				$sql .= " WHERE (site_id=1 OR site_id=" . $db->tosql($site_id, INTEGER) . ") ";
			} else {
				$sql .= " WHERE site_id=1 ";
			}
			$sql .= " AND (" . $where . ") ";
			...
			while ($db->next_record()) {
				...
				if (preg_match("/^products_list_(\d+)$/", $page_name, $matches)) {
					$layout_types[$matches[1]] = $setting_value;
				}
			}
		}
		...
		for ($c = (sizeof($ids) - 1); $c >= 0; $c--) {
			...
			if (isset($layout_types[$id])) {
				$layout_type = $layout_types[$id];
				if ($layout_type == "all") {
					$setting_name = "products_list_" . $id;
					break;
				} else if ($layout_type == "category" && $id == $category_id) {
					$setting_name = "products_list_" . $id;
					break;
				}
			}
	...
	$page_settings = va_page_settings($setting_name, 0);
