...
*/
	...
	$current_page = "order_info.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("order_info", $layout_id);

	$t = new VA_Template("./js");
	$t->set_file("ordering_js","ordering.js");
	$t->set_var("REQUIRED_PROPERTY_MSG", REQUIRED_PROPERTY_MSG);
	$t->set_var("REQUIRED_DELIVERY_MSG", REQUIRED_DELIVERY_MSG);
	$t->parse("ordering_js", false);
	$t->set_template_path($settings["templates_dir"]);
	$t->set_file("main","order_info.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "order_data_form") {
				$block_name = $setting_value;
				include_once("./blocks/block_order_info.php");
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", CHECKOUT_INFO_TITLE));
