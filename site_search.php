...
*/
	...
	$layout_id = 0;
	$page_settings = va_page_settings("site_search", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "site_search.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "site_search_results") {
				include_once("./blocks/block_site_search_results.php");
				site_search_results($setting_value);
	...
	$html_title = get_translation(get_setting_value($settings, "site_search_title", "Site Search Results"));
	$meta_description = get_translation(get_setting_value($settings, "site_search_description"));
	$meta_keywords = get_translation(get_setting_value($settings, "site_search_keywords"));
