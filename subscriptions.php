...
*/
	...
	$current_page = "subscriptions.php";
	$tax_rates = get_tax_rates();

	$layout_id = 0;
	$page_settings = va_page_settings("subscriptions", $layout_id);
	$confirm_add = get_setting_value($settings, "confirm_add", 1);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main", "subscriptions.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "subscriptions_breadcrumb") {
				include_once("./blocks/block_subscriptions_breadcrumb.php");
				subscriptions_breadcrumb($setting_value);
			} elseif ($setting_name == "subscriptions") {
				include_once("./blocks/block_subscriptions.php");
				subscriptions($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title", SUBSCRIPTIONS_MSG));
