...
if ($_GET["app"] == "motorcycle") {
	...
	$current_description = "<h2>Motorcycles, Small Recreation Vehicles and Watercraft</h2>
							<ol>
        						<li>Select your vehicle from the list below.<br /></li>
								<li>Choose the desired parts from the table that appears <strong>(they're all for your vehicle)</strong>.<br /></li>
								<li>Purchase your parts by checking the box next to the part and then clicking the <strong>ADD CHECKED ITEMS TO CART</strong> button.  You can also
									click on the part number to go the part's detail page. Your search results will be saved so you can come back to this page to review them.</li>
							</ol>";
	...
	$current_description = "<h2>Heavy Duty Applications</h2>
							...
							**note that some heavy duty applications have notes about the application.  To see any special notes, hold your 
	...
	$current_description = "<h2>Cross Reference</h2>
							...
							You can also click on the part number to go the part's detail page. Your search results will be saved so you can come back to this page to review them.";
} else {
...
if($_GET["app"] != "cross") { //Only show the make dropdown if it is not cross reference
	...
	$form_code .=  '	<div name="yeardiv" id="yeardiv" style="display: none">
					...
					<div style="float: right; padding-right: 10px;">
						<img src="/images/oil-filters/Free-Shipping-Over-50.jpg" alt="Autolite Spark Plugs" 
...
function showdiv($divid, $model, $make, $year, $engine) {
	...
	$application_code = '<div align="center">
						...
						<table width="675" border="0" cellspacing="0" cellpadding="2" style="margin-top:5px;background-color: black;color: silver;">
							<tr>
								<td width="50%"><div align="left" style="padding-left:5px"><strong>Part Type</strong></div></td>
								<td width="18%"><div align="left"><strong>Part #</strong></div></td>
								<td width="17%"><div align="center"><strong>Our Price</strong></div></td>
								<td width="15%"><div align="center"><strong>Add to Cart</strong></div></td>
							</tr>
						</table>
	...
	while ($rs=mysql_fetch_assoc($partRs)) {
		...
		if(is_null($rs["price"]) || $rs["Active"] == 0) {
			$application_code .= "<table width=\"665\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\" style=\"text-align:left\">
									<tr>
									<td width=\"50%\"><span class=\"".$manufacturer."\">".$manufacturer." </span><span>".$category."</span>";
								if ($description) {
			$application_code .= "&nbsp;<img src=\"images/info.gif\" title=\"".$description."\">";
								}
			$application_code .= "</td>
									<td width=\"15%\"><span>".$rs["part"]."</span></td>
									<td width=\"25%\">
										<div align=\"center\">
			...
			$application_code .= " </td>
									<td width=\"15%\">
									<a href=\"product_details.php?item_code=".$rs["part"]."\" target=\"_parent\" class=\"parts-highlight\">".$rs["part"]."
									</td>
									<td width=\"25%\"><div align=\"center\">$".$rs["price"]."</div></td>
									<td width=\"10%\">
									<div align=\"center\">
...
function getcross($partnumsubmit) {
	...
	if (mysql_num_rows($crossRs) > 0){
			...
			while ($rs=mysql_fetch_assoc($crossRs)) {
				$application_code .= '<table width="665" border="0" cellspacing="0" cellpadding="2" style="padding-top:5px">
										<tr>
											...
											<td width="15%">
												<div align="center">
													<a href="product_details.php?item_code='.$rs["Part"].'" class="parts-highlight">'.$rs["Part"].'</a>
												</div>
											</td>';
				if(is_null($rs["price"])) {
			...
			$application_code = '<br />
								<div align="center">
									<span>Sorry, there were no matches found.  Try your search again with only a segment of the part number.</span>
								</div>';
			$objResponse->assign("submitdiv", "innerHTML", $application_code);
	}
$_SESSION["application"] = $application_code;
...
<script type="text/javascript">
	...
	function disableEnterKey(e) {
		 ...
		 if(key == 13) {
			document.getElementById("submitdiv").innerHTML = '<div align="center"><img src="images/loading.gif" style="margin-top:50px;padding-bottom:10px"><br /><span>Please Wait While We Find Your Part</span></div>';
