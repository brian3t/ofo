...
*/
	...
	$current_page = "product_options.php";
	$tax_rates = get_tax_rates();
	$confirm_add = get_setting_value($settings, "confirm_add", 1);

	$layout_id = 0;
	$page_settings = va_page_settings("products_options", $layout_id);

	$t = new VA_Template($settings["templates_dir"]);
	$t->set_file("main","product_options.html");
	...
	if (is_array($page_settings)) {
		foreach ($page_settings as $setting_name => $setting_value)
		{
			...
			} elseif ($setting_name == "products_options") {
				include_once("./blocks/block_product_options.php");
				product_options($setting_value);
	...
	$html_title = get_translation(get_setting_value($page_settings, "html_title"));
